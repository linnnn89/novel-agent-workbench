...
- project config policy defaults for enabling style checks, calibration, hint display, and future UI placement,
- disabling style checks by project policy,
- hiding hints or disabling calibration through call/CLI overrides,
...
- Planning Library creation, duplicate/secret-like rejection, metadata-only default list/read/state/CLI output, explicit text inclusion, active-reference tracking, per-item delete cleanup, context package/prompt render integration, inactive item skipping, and no Provider/draft/confirmed chapter/Memory Bank/RAG/export side effects.
...
- desktop chapter regeneration prompt behavior: `要求重写（重新随机）` treats the target chapter as unwritten, excludes the previous draft body, and leaves old-draft-based fixes to the AI review refinement path.
