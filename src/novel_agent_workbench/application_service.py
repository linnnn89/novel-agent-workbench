...
from .project_state import public_project_state, project_overview
...
# Keep the existing helper imports available to callers during the module split.
from .refinement import (
    DraftRefinementService, AI_REFINEMENT_SYSTEM_PROMPT,
    estimate_refinement_input_tokens, refinement_capacity_check,
    ai_refinement_system_prompt, ai_refinement_task_prompt, render_ai_refinement_prompt,
)
...
class WorkbenchApplicationService:
    ...
    def project_overview(self, project_id: str) -> dict[str, Any]:
        return project_overview(self._runtime_store(project_id))
    ...
    def refine_draft_from_ai_review(
        ...
        *,
        ...
        store = self._runtime_store(project_id)
        settings, _ = self._effective_project_generation_settings(store)
        return DraftRefinementService(store, generation_settings=settings).refine_draft(
            draft_id,
            review_id=review_id,
    ...
    def set_memory_item_enabled(
        ...
        *,
        ...
        return MemoryBankService(self._open_store(project_id)).set_memory_item_enabled(
            ...
            reason_code=reason_code,
            target_token_budget=target_token_budget,
            checkpoint_before_update=checkpoint_before_update,
        ).to_dict()
