...
- `providers.py`: writer/scorer/reviser config parsing, Provider adapter registry, `project_secret.*` resolver, safe config/secret write helpers, fake/no-network status checks, dry-run request summary translation, explicit Chutes real-test metadata path, Provider request/response interface, deterministic Mock Provider, disabled real-provider placeholders including Chutes, and safe provider call logging.
