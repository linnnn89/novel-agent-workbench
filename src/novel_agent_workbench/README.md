...
- `providers.py`: writer/scorer/reviser config parsing, Provider adapter registry, `project_secret.*` resolver, safe config/secret write helpers, fake/no-network status checks, Provider request/response interface, deterministic Mock Provider, disabled real-provider placeholders, and safe provider call logging.
